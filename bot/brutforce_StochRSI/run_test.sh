...
Y1=106 #interval
Y2=26 #thresholds low
Y3=94 #thresholds high
Y4=6 #thresholds persistence
...
if [[ $Y4 -lt $Y4_low ]]; then Y4=$Y4_low 
fi
...
if [ $(echo "$RESULT >= $MAX_RESULT" | bc) -eq 1 ]; then	
		...
		echo interval = $Y1 >> "$test_name"_result.log
		echo [thresholds] >> "$test_name"_result.log
		echo low =  $Y2 >> "$test_name"_result.log
		echo high =  $Y3 >> "$test_name"_result.log
		echo persistence =  $Y4 >> "$test_name"_result.log
