...
var pipeline = (settings) => {
  ...
  var referenceEmitters = function (next) {
    _.each(plugins, function (plugin) {
      if (plugin.meta.emits) { emitters[plugin.meta.slug] = plugin }
    })
  ...
  var subscribePlugins = function (next) {
    ...
    _.each(plugins, function (plugin) {
      _.each(marketSubscriptions, function (sub) {
        if (plugin[sub.handler]) {
          if (sub.event === 'candle') { candleConsumers.push(plugin) }
        }
      })
    })
  ...
  var prepareMarket = function (next) {
    if (mode === 'backtest' && config.backtest.daterange === 'scan') { require(dirs.core + 'prepareDateRange')(next) } else { next() }
  }

  var setupMarket = function (next) {
    // load a market based on the config (or fallback to mode)
    let marketType
    if (config.market) { marketType = config.market.type } else { marketType = mode }
