...
var util = {
  getConfig: function () {
    ...
    if (!fs.existsSync(util.dirs().root + program.config)) { util.die('Cannot find the specified config file.', true) }
  ...
  setConfigProperty: function (parent, key, value) {
    if (parent) { _config[parent][key] = value } else { _config[key] = value }
  },
  getVersion: function () {
  ...
  gekkoMode: function () {
    ...
    if (program.import) { return 'importer' } else if (program.backtest) { return 'backtest' } else { return 'realtime' }
  },
  gekkoModes: function () {
    return [
      'importer',
  ...
  launchUI: function () {
    if (program.ui) { return true } else { return false }
  },
