...
Checker.prototype.cantMonitor = function (conf) {
  ...
  if (!exchange) { return 'Gekko does not support the exchange ' + slug }
  ...
  if (!_.includes(exchange.assets, conf.asset)) { return 'Gekko only supports the assets [ ' + exchange.assets.join(', ') + ' ]  at ' + name + ' (not ' + conf.asset + ')' }

  var pair = _.find(exchange.markets, function (p) {
    return p.pair[0] === conf.currency && p.pair[1] === conf.asset
  })

  if (!pair) { return 'Gekko does not support this currency/assets pair at ' + name }
...
Checker.prototype.cantFetchFullHistory = function (conf) {
  ...
  if (this.cantMonitor(conf)) { return this.cantMonitor(conf) }
...
Checker.prototype.cantTrade = function (conf) {
  ...
  if (cantMonitor) { return cantMonitor }

  var slug = conf.exchange.toLowerCase()
  var exchange = this.getExchangeCapabilities(slug)
  var name = exchange.name

  if (!exchange.tradable) { return 'At this moment Gekko can\'t trade at ' + name + '.' }

  if (conf.key === 'your-key') { return '"your-key" is not a valid API key' }

  if (conf.secret === 'your-secret') { return '"your-secret" is not a valid API secret' }

  var error = false
  _.each(exchange.requires, function (req) {
    if (!conf[req]) { error = name + ' requires "' + req + '" to be set in the config' }
  }, this)
