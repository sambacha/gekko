...
request(options)
  .then(body => {
    ...
    const assets = _.uniq(_.map(body.data, market => market.name.split('-')[0]))
    const currencies = _.uniq(_.map(body.data, market => market.name.split('-')[1]))
    const pairs = _.map(body.data, market => {
