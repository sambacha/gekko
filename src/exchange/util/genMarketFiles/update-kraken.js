...
const assetPairsPromise = request({
...
Promise.all([assetPromise, assetPairsPromise])
  .then(results => {
    const assets = _.uniq(_.map(results[1], market => {
      return results[0][market.base].altname
    }))

    const currencies = _.uniq(_.map(results[1], market => {
      return results[0][market.quote].altname
    }))

    const marketKeys = _.filter(_.keys(results[1]), k => { return !k.endsWith('.d') })
    const markets = _.map(marketKeys, k => {
      const market = results[1][k]
      const asset = results[0][market.base]
      const currency = results[0][market.quote]
      return {
