...
Trader.prototype.getTicker = function (callback) {
  const setTicker = (err, data) => {
    ...
    if (!result) { return callback(new Error(`Market ${this.pair} not found on Binance`)) }
...
Trader.prototype.getOrder = function (order, callback) {
  const get = (err, data) => {
    ...
    _.each(trades, trade => {
      ...
      if (fees[trade.commissionAsset]) { fees[trade.commissionAsset] += (+trade.commission) } else { fees[trade.commissionAsset] = (+trade.commission) }
    })
