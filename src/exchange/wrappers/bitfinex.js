...
Trader.prototype.getPortfolio = function (callback) {
  const processResponse = (err, data) => {
    ...
    if (_.isObject(asset) && _.isNumber(+asset.available) && !_.isNaN(+asset.available)) { assetAmount = +asset.available } else {
      assetAmount = 0
    }

    if (_.isObject(currency) && _.isNumber(+currency.available) && !_.isNaN(+currency.available)) { currencyAmount = +currency.available } else {
      currencyAmount = 0
    }
...
Trader.prototype.getTrades = function (since, callback, descending) {
  ...
  var path = this.pair
  if (since) { path += '?limit_trades=2000' }
