...
Trader.prototype.handleResponse = function (funcName, callback) {
  return (error, body) => {
    if (!error) {
      if (_.isEmpty(body)) { error = new Error('NO DATA WAS RETURNED') } else if (!_.isEmpty(body.error)) { error = new Error(body.error) }
    }
...
Trader.prototype.getTicker = function (callback) {
  ...
  const handler = (cb) => this.coingi.api('order-book', '/' + this.pair + '/1/1/1', this.handleResponse('getTicker', cb))
...
Trader.prototype.getTrades = function (since, callback, ascending) {
  ...
  var processResults = function (err, trades) {
    ...
    if (ascending) { callback(undefined, parsedTrades) } else { callback(undefined, parsedTrades.reverse()) }
  }
  ...
  const handler = (cb) => this.coingi.api('transactions', '/' + this.pair + '/512' + optionalSince, this.handleResponse('getTrades', cb))
...
Trader.prototype.getPortfolio = function (callback) {
  ...
  const handler = (cb) => this.coingi.api('balance', { currencies: this.asset + ',' + this.currency }, this.handleResponse('getPortfolio', cb))
...
Trader.prototype.addOrder = function (tradeType, amount, price, callback) {
  ...
  const handler = (cb) => this.coingi.api('add-order', reqData, this.handleResponse('addOrder', cb))
...
Trader.prototype.getOrder = function (orderId, callback) {
  ...
  const reqData = { ordeId: orderId }
  const handler = (cb) => this.coingi.api('get-order', reqData, this.handleResponse('getOrder', cb))
...
Trader.prototype.checkOrder = function (orderId, callback) {
  ...
  const handler = (cb) => this.coingi.api('get-order', reqData, this.handleResponse('checkOrder', cb))
  util.retryCustom(retryCritical, _.bind(handler, this), _.bind(check, this))
}

Trader.prototype.cancelOrder = function (order, callback) {
  const reqData = { orderId: order }
  const handler = (cb) => this.coingi.api('cancel-order', reqData, this.handleResponse('cancelOrder', cb))
