...
Pushbullet.prototype.processTradeCompleted = function (trade) {
  ...
  const sendOnTrade = pbConf.sendOnTrade === undefined ? 1 : pbConf.sendOnTrade
  ...
  if (sendOnTrade) {
    ...
    if (trade.action === 'buy') {
      ...
      const oBal = this.lastBuyBalance // Old Balance
      const nBal = trade.balance // New Balance
      const diffBal = Math.abs(nBal - oBal) // Balance Difference
      const percDiffBal = (diffBal / oBal) * 100 // Percentage difference
      ...
      const sBal = this.startingBalance
      const tDiffBal = Math.abs(nBal - sBal)
      const percDiffTotBal = (tDiffBal / sBal) * 100
      if (nBal >= sBal) { // profit!
    ...
    if (!config.paperTrader.enabled) {
      const timeToComplete = moment.duration(trade.date.diff(this.adviceTime)).humanize()
...
Pushbullet.prototype.checkResults = function (err) {
  if (err) { log.warn('error sending pushbullet message', err) } else { log.info('Send advice via pushbullet message.') }
}
