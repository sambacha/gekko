...
*/
  ...
  let fileContent = fs.readFileSync(fileName, "utf8");
        ...
        signal_sell_pos = fileContent.indexOf("15M_SUM_SELL");
        signal_sell = fileContent.substring(signal_sell_pos+14, signal_sell_pos+16); 
        signal_sell_int = Number.parseInt(signal_sell);
        log.debug("15M_SUM_SELL "+signal_sell_pos+" "+signal_sell_int);

        signal_neutral_pos = fileContent.indexOf("15M_SUM_NEUTRAL");
        signal_neutral = fileContent.substring(signal_neutral_pos+17, signal_neutral_pos+19); 
        signal_neutral_int = Number.parseInt(signal_neutral);
        log.debug("15M_SUM_NEUTRAL "+signal_neutral_pos+" "+signal_neutral_int);

        signal_buy_pos = fileContent.indexOf("15M_SUM_BUY");
        signal_buy = fileContent.substring(signal_buy_pos+13, signal_buy_pos+15); 
        signal_buy_int = Number.parseInt(signal_buy);
        log.debug("15M_SUM_BUY "+signal_buy_pos+" "+signal_buy_int);
